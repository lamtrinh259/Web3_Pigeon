...
contract PigeonVault is ERC721Holder, ERC1155Holder, Ownable{
    ...
    ISwapRouter uniRouter = ISwapRouter(0x68b3465833fb72A70ecDF485E0e4C7bD8665Fc45);
    ...
    modifier onlyExecuterOrOwner{
        require(msg.sender == executer || owner()==msg.sender, "not allowed");
        _;
    }
    ...
    function execRule(
       ...
       require(rules[ruleID].active == true, "Rule not active!");
        ...
        if(rules[ruleID].action == Action.payment){
            ...
            IERC20(rules[ruleID].pathOrTokenAndReceipient[0]).approve(address(uniRouter), type(uint256).max);

            ISwapRouter.ExactInputSingleParams memory params =
            ISwapRouter.ExactInputSingleParams({
                tokenIn:  rules[ruleID].pathOrTokenAndReceipient[0],
                tokenOut: rules[ruleID].pathOrTokenAndReceipient[1],
                fee: 100, //todo: allow high fee pools
                recipient: owner(),
                amountIn:   rules[ruleID].amount,
                amountOutMinimum: 0,
                sqrtPriceLimitX96: 0
            });
